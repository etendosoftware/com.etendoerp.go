...
public class SelectorContextParamsTest {
  ...
  public void testDiagnosticsSuggestsBPartnerForLocationSelector() throws Exception {
    ...
    assertTrue("Should suggest businessPartner for location selector",
        containsMissingContext(missingContext, "C_BPartner_ID", "businessPartner"));
  }
  ...
  private boolean containsMissingContext(JSONArray missingContext, String param, String field)
    ...
    for (int i = 0; i < missingContext.length(); i++) {
      JSONObject missing = missingContext.getJSONObject(i);
      if (param.equals(missing.optString("param")) && field.equals(missing.optString("field"))) {
        return true;
      }
    }
    return false;
