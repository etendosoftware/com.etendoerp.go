...
public class AmortizationPlanServiceTest {
  ...
  private static Asset mockDepreciableAsset(boolean isDepreciate, String calculateType, Long usableLifeMonths) {
    Asset asset = mock(Asset.class);
  ...
  public void testPeAssetDoesNotRejectForNullUsableLife() {
    ...
    try (MockedStatic<OBDal> obDalMock = mockStatic(OBDal.class)) {
      ...
      assertNotEquals("PE asset must not be rejected with 400 for null usable life", 400, response.getHttpStatus());
    }
  }
  ...
  public void testProcessExecutionErrorIsPropagated() {
    ...
    try (MockedStatic<OBDal> obDalMock = mockStatic(
        OBDal.class); MockedStatic<NeoProcessService> processMock = mockStatic(NeoProcessService.class)) {
      obDalMock.when(OBDal::getInstance).thenReturn(dalMock);
      processMock.when(
          () -> NeoProcessService.executeProcess(any(Process.class), isNull(), eq(ASSET_ID), eq(TAB_ID))).thenReturn(
          processErrorResponse);

      NeoResponse response = AmortizationPlanService.generatePlan(ASSET_ID);

      assertTrue("Process error must be propagated with status >= 400", response.getHttpStatus() >= 400);
    }
  }
  ...
  public void testSuccessPathTiReturnsCorrectJson() throws Exception {
    ...
    try (MockedStatic<OBDal> obDalMock = mockStatic(
      ...
      assertEquals(new BigDecimal("200.00"), new BigDecimal(body.getString("totalAmortization")));
      assertEquals("EUR", body.getString("currency"));
      assertEquals(2, body.getInt("periodsGenerated"));
      assertEquals(new BigDecimal("100.00"), new BigDecimal(body.getString("periodAmount")));
  ...
  public void testNullCurrencyOmitsFieldWithoutNpe() throws Exception {
    ...
    try (MockedStatic<OBDal> obDalMock = mockStatic(
      ...
      assertEquals(200, response.getHttpStatus());
      assertFalse("currency field must be absent when currency is null", response.getBody().has("currency"));
    }
  }
  ...
  public void testUnexpectedExceptionReturns500() {
    OBDal dalMock = mock(OBDal.class);
    when(dalMock.get(Asset.class, ASSET_ID)).thenThrow(new RuntimeException("Simulated DB failure"));
  ...
  public void testNullLineAmountTreatedAsZero() throws Exception {
    ...
    try (MockedStatic<OBDal> obDalMock = mockStatic(
      ...
      assertEquals(200, response.getHttpStatus());
      assertEquals(new BigDecimal("0").compareTo(new BigDecimal(response.getBody().getString("totalAmortization"))), 0);
      assertEquals(new BigDecimal("0").compareTo(new BigDecimal(response.getBody().getString("periodAmount"))), 0);
    }
  }
}
