...
public class ReconciliationHandlerTest {
  ...
  public void testBuildAutoMatchRuleFallbackCreatesGroup() throws Exception {
    ...
    try (MockedStatic<OBDal> obDal = mockStatic(OBDal.class);
      ...
      // Seven arguments: buildAutoMatch calls the overload taking a MatchSettings. Stubbing a
      // different arity does not fail loudly — the call simply misses the stub and Mockito hands
      // back a null int[], which then NPEs on `delta[0]` inside buildAutoMatch, far from the cause.
      ams.when(() -> AutoMatchSupport.matchFallback(any(), any(), any(), any(), any(), any(), any()))
          .thenCallRealMethod();
