 ...
 * accounting dimensions are active right now", for every consumer of the
 * {@code FIN_Finacc_Transaction} entity (ETP-4950, narrowed by ETP-5101).
 *
 * <p><b>ETP-5101:</b> the class used to also read Core's centrally-maintained,
 * per-{@code docBaseType}/level dimension matrix ({@code AD_Client_AcctDimension}) as an
 * alternative source, selected by {@code AD_Client.Acctdim_Centrally_Maintained}. That machinery
 * (and its tests) was removed entirely per explicit product direction: a
 * {@code FIN_Finacc_Transaction} must be governed by the exact same flat, per-tenant switch every
 * other GO window uses — {@code C_AcctSchema_Element.IsActive} — never a document-type-scoped
 * override. See the class javadoc for the fuller history. What remains is the flat source itself
 * ({@code flatActiveDimensionsForAccount} / {@code flatActiveDimensionsForClient}) and the two
 * consumers of its answer: {@code applyRuleDimensions}, which copies a matching rule's project /
 * cost center / product onto the {@code FIN_FinaccTransaction} Automatch generates (skipping
 * whatever is not active, and resolving the allowed set LAZILY — through a {@link Supplier} that
 * must stay unconsulted for a dimensionless spec), and {@code toOrderedArray}, which serializes a
 * dimension set in the canonical display order.
 *
 * <p>Both flat queries are driven through a mocked JDBC {@link Connection}, told apart by whether
 * they scope through {@code fin_financial_account}. JUnit 4 + the Silent runner mirror the sibling
 * handler tests, and {@code clearInlineMocks()} keeps the module's single test JVM heap flat.
 */
@RunWith(MockitoJUnitRunner.Silent.class)
public class AccountingDimensionsSupportTest {

  /** Table fragment used to tell the two prepared statements apart. */
  ...
  /**
   * Stubs one of the two flat queries on {@code conn} and returns its statement (so a test can
   * assert the bound parameters).
   *
   * @param accountScoped whether the wanted variant resolves the tenant through the account
   */
  private static PreparedStatement stubQuery(Connection conn, boolean accountScoped,
      String column, String... codes) throws Exception {
    PreparedStatement ps = psRows(column, codes);
    when(conn.prepareStatement(argThat((String sql) -> sql != null && sql.contains(TBL_ELEMENT)
  ...
  public void testFlatActiveDimensionsForAccountMapsKnownElementCodes() throws Exception {
    try (MockedStatic<OBDal> obDal = mockStatic(OBDal.class)) {
      ...
      dalOn(obDal, conn);
      stubQuery(conn, true, COL_ELEMENT, "PJ", " CC ", "PR", "AC");
  ...
  public void testFlatActiveDimensionsForClientBindsTheClientIdAsString() throws Exception {
    try (MockedStatic<OBDal> obDal = mockStatic(OBDal.class)) {
      Connection conn = mock(Connection.class);
      dalOn(obDal, conn);
      PreparedStatement ps = stubQuery(conn, false, COL_ELEMENT, "U1", "U2");
  ...
  public void testFlatActiveDimensionsWithNoRowsReturnsEmptySet() throws Exception {
    try (MockedStatic<OBDal> obDal = mockStatic(OBDal.class)) {
      Connection conn = mock(Connection.class);
      dalOn(obDal, conn);
      stubQuery(conn, false, COL_ELEMENT);
