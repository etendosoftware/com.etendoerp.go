...
| `com.etendoerp.go.schemaforge.email` | Transactional email framework, provider adapter boundary, contract SPI, safety controls, and current built-in contract providers. | Keep the framework contract-driven. New document families must provide contracts through `EmailContractProvider` and inject their own `EmailDocumentRecordResolver`; do not add document-specific branches or methods to shared framework resolvers. |
