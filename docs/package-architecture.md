...
| `com.etendoerp.go.featureflags` | Backend feature-flag evaluation: the OpenFeature entry point, its Mixpanel provider wiring, and the vendor-neutral targeting context. | Application code must depend only on `GoFeatureFlags` and `FeatureFlagContext`; no OpenFeature or Mixpanel type may leak outside this package. See `feature-flags-and-tenant-upgrade.md`. |
