...
**Exception — List reference columns (AD_Reference_ID=17):** a literal without `@` on a List column is resolved locally, without going through `Utility.getDefault()`. AD_Ref_List values are opaque codes that are sometimes all-digit strings (e.g. `Invoicegrouping` on `C_BPartner`, a 15-digit binary code like `"000000000000000"`), and the post-resolution numeric coercion collapsed them to `"0"` — losing the leading zeros / length and producing a value that matches no real `AD_Ref_List` entry. For that reason `applyResolvedDefault` (post-resolution BigDecimal/Long coercion) and `NeoTypeCoercionHelper.coerceField` (coercion on the create payload) both exclude List columns from their numeric coercion, even though `Property.getPrimitiveObjectType()` reports a numeric type for them (ETP-4700). Note that `Utility.getDefault()` itself performs no numeric coercion — it returns a literal without `@` verbatim; the collapse came from those two coercion points, not from it.

**One pair of enclosing quotes is stripped from that literal** (`stripEnclosingQuotes`). Quoting a string literal in `AD_Column.DefaultValue` is the standard AD convention — the core strips it in `Property.getActualDefaultValue()`, and `Utility.getDefault()` does not — so a List default stored as `'1'` (e.g. `C_BPartner.EM_OBTIK_Tax_ID_Key`, "Tipo de documento fiscal") must be served as `1`. Returning it verbatim made every business-partner create fail with a 400 on that field, since no `AD_Ref_List` entry matches `'1'`. The stripping requires the SAME quote character at both ends and a string longer than the two quotes, which is what leaves the unquoted codes above (`000000000000000`) untouched.
