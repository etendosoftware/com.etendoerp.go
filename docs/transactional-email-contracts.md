...
- **Eight contracts record, five windows display.** `DefaultDocumentSendEmailContract` opts in all eight document sends — sales-order, purchase-order, sales-quotation, sales-invoice, goods-shipment, **return-to-vendor**, and, since ETP-5124, **return-material-receipt** and **return-to-vendor-shipment** — but the app-shell's `EmailsCard` is currently wired into only four preview components covering five windows (`OrderPreview` serves sales-order and purchase-order, plus `InvoicePreview`, `QuotationPreview`, `GoodsShipmentPreview`). Return-to-vendor, return-material-receipt and return-to-vendor-shipment sends are recorded and readable through the backoffice window and the endpoint; they simply have no preview card yet. Nothing needs to change on the backend when one is added.
...
The document-send family (`sales-invoice-send` and its seven siblings) reads its ceilings from
...
| `return-material-receipt-send` | `custom` | `C_BPartner.EM_Etgo_Email`, falling back to active contact email, resolved from the return receipt business partner | `version`, `recordId` |
| `return-to-vendor-shipment-send` | `custom` | `C_BPartner.EM_Etgo_Email`, falling back to active contact email, resolved from the return shipment business partner | `version`, `recordId` |
