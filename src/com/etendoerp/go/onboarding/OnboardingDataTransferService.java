...
public class OnboardingDataTransferService {
  ...
  public TransferResult transfer(String sourceClientId, String targetClientId, String targetOrgId,
    ...
    try {
      ...
      String targetPriceListVersionId = products
          ? findTargetSalesPriceListVersionId(targetClientId)
          : null;
  ...
  private String findTargetSalesPriceListVersionId(String clientId) {
    OBQuery<PriceListVersion> query = OBDal.getInstance().createQuery(PriceListVersion.class,
        "as v where v.priceList.client.id = :clientId and v.priceList.salesPriceList = true "
            + "and v.active = true order by v.priceList.default desc, v.validFrom desc, v.id");
    query.setNamedParameter("clientId", clientId);
    query.setFilterOnReadableClients(false);
    query.setFilterOnReadableOrganization(false);
    query.setMaxResult(1);
    PriceListVersion version = query.uniqueResult();
