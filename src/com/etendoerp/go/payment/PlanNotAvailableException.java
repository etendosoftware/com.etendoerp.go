...
/**
 * Signals that the plan key the browser asked to buy names no active catalog row.
 *
 * <p>Deliberately one exception for two causes — an unknown key and an inactive one. The endpoint
 * that reports it must not let a caller enumerate the catalog by probing keys, the same
