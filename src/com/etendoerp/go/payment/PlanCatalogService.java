...
/**
 * Reads the plan catalog ({@code ETGO_PLAN}) — the rows that replaced the single deployment
 * property {@code etendo.go.checkout.price.id} as the answer to "what can be bought".
 *
...
public class PlanCatalogService {
  ...
  /**
   * Finds the catalog row a checkout may be started from.
   *
   * <p><b>What this method decides, and what it deliberately does not.</b> It answers "is there an
   * active catalog row under this key" and nothing else. Whether that row carries a provider price
   ...
   *       plan — → {@code 503 CHECKOUT_NOT_CONFIGURED}, because the catalog is fine and the
   ...
   * @return the active catalog row, or {@link Optional#empty()} when there is none
   */
  ...
  /**
   * Lists every catalog row a checkout may actually be started from, cheapest first.
   *
  ...
  /**
   * Returns whether a catalog row can actually be charged for.
   *
   ...
   *
   * @param plan a catalog row, may be null
