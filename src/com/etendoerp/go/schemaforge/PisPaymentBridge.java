...
import com.etendoerp.psd2.bank.integration.utils.PISPaymentOrchestrator;

/**
 * Bridges a NEO-registered {@link FIN_Payment} to the PSD2 module's PIS (Payment Initiation
 * Service) orchestrator, so paying a purchase invoice by real bank transfer from the Etendo Go
 * "Add payment" modal goes through the same Salt Edge flow as Classic's "Generate Bank Payment"
 ...
 * normalizes before calling its own {@code processPayment}: {@code template}, {@code
 * end_to_end_id}, {@code creditor_name}, {@code amount}, {@code currency_id}, {@code
 * description}, {@code creditor_iban}. Template selection is currency-driven — {@code SEPA} for
 * EUR, {@code FPS} for GBP — any other currency must already have been rejected by the caller's
 * eligibility check (see {@code PaymentRegistrationService#validatePisEligibility}) before this
 * class is reached.
 */
final class PisPaymentBridge {
  ...
  /**
   * Builds the PIS payment request for {@code payment} and delegates to the shared PSD2
   * orchestrator, returning the Salt Edge payment id + SCA widget URL.
   *
   ...
   * compatibility. The orchestrator validates which creditor fields are required per template.
   *
   ...
   * @param request  the current HTTP request (used by the orchestrator to resolve the client's
  ...
  static BankIntegrationPISUtils.PISCreatePaymentResult initiatePisPayment(FIN_Payment payment,
    ...
    return PISPaymentOrchestrator.createPaymentForFinPayment(payment, params, apiKey, request);
  }
