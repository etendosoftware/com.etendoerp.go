...
public class FinancialAccountTransactionsHandler implements NeoHandler {
  ...
  /**
   * Business rejection for {@code ?action=delete} on a movement that belongs to a payment
   * (see {@link #handleDelete}). The correct direction is payment &rarr; transaction, so such a
   * movement can only be removed by removing its {@code FIN_Payment}; deleting it here would leave
   * the payment without its bank transaction. Kept in ENGLISH and byte-for-byte in sync with the
   * frontend's {@code BACKEND_ERROR_MAP} key {@code backendError.paymentMovementNotDeletable}
   * (lib/backendErrors.js), which matches it by EXACT text after {@code .trim()} — rewording this
   * string silently drops users back to English.
   */
  private static final String MSG_PAYMENT_LINKED_NOT_DELETABLE =
      "This movement belongs to a payment. Delete it from the payment instead.";
  /**
   * Same rejection as {@link #MSG_PAYMENT_LINKED_NOT_DELETABLE}, for a movement whose
   * {@code FIN_Payment} is a RECEIPT (cobro) rather than a payment (pago) — chosen off
   * {@code FIN_Payment.isReceipt()} in {@link #handleDelete}. Kept in ENGLISH and byte-for-byte in
   * sync with the frontend's {@code BACKEND_ERROR_MAP} key
   * {@code backendError.receiptMovementNotDeletable} (lib/backendErrors.js), which matches it by
   * EXACT text after {@code .trim()} — rewording this string silently drops users back to English.
   */
   ...
   * trigger violation is a JDBC error, NOT an {@code OBException}, so it escapes
   * {@link FinancialAccountTransactionsSupport#runMutation}'s business-error branch and surfaces as
   * an opaque HTTP 500 that the frontend discards. Kept in ENGLISH and byte-for-byte in sync with
   * the frontend's {@code BACKEND_ERROR_MAP} key
   * {@code backendError.movementProcessedNotDeletable} (lib/backendErrors.js), which matches it by
   * EXACT text after {@code .trim()}.
   */
  private static final String MSG_PROCESSED_NOT_DELETABLE =
   ...
   *
   * <p>Body: {@code { "id": "...", "paymentRemoval": true|false }}. The optional
   * {@code paymentRemoval} flag says whether a Processed movement may be reactivated and removed
   * (Classic's REMOVE TRANSACTION) or must be rejected. It <b>defaults to {@code true}</b> so the
   * REST API, MCP and the movements kebab keep their existing behaviour; the bulk-delete UI sends
   * {@code false} when more than one row is selected, because in that case the reason cannot be
   * explained per row (ETP-5111).
   *
   * <p>Three up-front 409 rejections, in this order:
   * <ol>
   *   <li><b>Belongs to a payment or a receipt</b> (the wording is picked off
   *       {@code FIN_Payment.isReceipt()}) — the correct direction is payment &rarr; transaction
   *       ({@code PaymentRemovalUtil} drives {@code TransactionRemovalUtil}, never the reverse), so
   *       deleting the movement here would strand the {@code FIN_Payment} without its bank
   *       transaction. Until ETP-5111 this was unguarded on the REST/MCP/bulk paths.</li>
   *   <li><b>Leg of a funds transfer</b>
   *       ({@link FinancialAccountTransactionsSupport#isTransferCounterpart}): its counterpart
   *       references it through a RESTRICT self-FK, so the removal could only ever fail — and it
   *       failed as an opaque HTTP 500, because the JDBC constraint violation raised at flush time
   *       is not an {@code OBException} (ETP-5085).</li>
   *   <li><b>Processed with {@code paymentRemoval:false}</b> — mirrors core's
   *       {@code APRM_FIN_FINACC_TRAN_CHECK_TRG}, which makes a plain DELETE of a processed
   *       movement impossible at DB level. Pre-checking it keeps the answer a deterministic,
   *       translatable 409 instead of the same opaque 500 trap as above.</li>
   * </ol>
   *
   * <p>All three are the server-side enforcement for the bulk path, the REST API and MCP: the UI no
   * longer disables the delete affordance, it lets the attempt through and explains the failure.
   */
  private NeoResponse handleDelete(NeoContext context) {
    ...
    return FinancialAccountTransactionsSupport.runMutation(body, ACTION_DELETE,
        "Could not delete the movement. Please check logs for details.", () -> {
          // Defaults to true: REST, MCP and the row kebab delete a single movement and expect the
          // payment-removal path. Only the bulk UI opts out, by sending false for a multi-row
          // selection (ETP-5111).
          boolean allowPaymentRemoval = body.optBoolean("paymentRemoval", true);
          FIN_FinaccTransaction trx = loadTransactionFromBody(body);
          if (trx == null) return NeoResponse.error(404, MSG_TRANSACTION_NOT_FOUND);
          // 409, mirroring FinancialAccountHandler.deleteAccount's own "cannot delete" rejection.
          if (trx.getFinPayment() != null) {
            // Boxed Boolean: isReceipt() is null on a payment whose flag was never set, so it is
            // unwrapped rather than dereferenced (same convention as isProcessed() below).
            return NeoResponse.error(409,
                Boolean.TRUE.equals(trx.getFinPayment().isReceipt())
                    ? MSG_RECEIPT_LINKED_NOT_DELETABLE
                    : MSG_PAYMENT_LINKED_NOT_DELETABLE);
          }
          ...
          if (Boolean.TRUE.equals(trx.isProcessed())) {
            if (!allowPaymentRemoval) {
              return NeoResponse.error(409, MSG_PROCESSED_NOT_DELETABLE);
            }
