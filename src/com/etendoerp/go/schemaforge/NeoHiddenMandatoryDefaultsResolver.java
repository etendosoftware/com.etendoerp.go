...
final class NeoHiddenMandatoryDefaultsResolver {
  ...
  private static boolean shouldResolveColumn(Request request, Column column, String dbColumnName) {
    return Boolean.TRUE.equals(column.isActive())
        && Boolean.TRUE.equals(column.isMandatory())
  ...
  private static void resolveProperty(Request request, Column column, String dbColumnName,
    ...
    Object valueToPut = resolved instanceof BaseOBObject
        ? ((BaseOBObject) resolved).getId()
        : resolved;
    request.defaults.put(propertyName, valueToPut);
    if (property != null && request.dalEntity != null && request.identifierInjector != null) {
      request.identifierInjector.inject(request.defaults, request.dalEntity, propertyName,
          valueToPut);
    }
    log.debug("Resolved hidden mandatory default: {} = {}", propertyName, valueToPut);
  }
  ...
  static final class Request {
    ...
    Request(JSONObject defaults, Entity dalEntity, Tab adTab) {
      if (defaults == null) {
        throw new IllegalArgumentException("defaults JSONObject cannot be null");
      }
