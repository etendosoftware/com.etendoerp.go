...
public class WidgetBestProductsHandler implements NeoHandler {
  ...
  private List<Object[]> queryWithRange(String clientId, String range) {
    String sql = String.format(BEST_PRODUCTS_RANGED, rangeToSqlDateFrom(range));
  ...
  private List<Object[]> queryFallback(String clientId) {
    NativeQuery<Object[]> query = OBDal.getInstance().getSession().createNativeQuery(BEST_PRODUCTS_FALLBACK);
    query.setParameter("clientId", clientId);
    return query.list();
  }
