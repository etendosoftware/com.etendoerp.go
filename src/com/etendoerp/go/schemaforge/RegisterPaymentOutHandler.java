 ...
 * creates a FIN_Payment (payment out / APP), links it to the PSDs, and processes it.
 */
...
public class RegisterPaymentOutHandler implements NeoHandler {
  ...
  private NeoResponse doRegisterPayment(String invoiceId, String scheduleId,
    ...
    DocumentType docType = FIN_Utility.getDocumentType(org, "APP");
    if (docType == null) {
      throw new OBException("Document type for Payments (APP) not found for the organization.");
    }
    ...
    FIN_Payment payment = new AdvPaymentMngtDao().getNewPayment(
        false, org, docType, docNo, bp, paymentMethod, account,
