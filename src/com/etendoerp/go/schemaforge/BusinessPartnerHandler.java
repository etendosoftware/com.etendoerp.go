...
public class BusinessPartnerHandler implements NeoHandler {
  ...
  private void deriveNameFromPerson(NeoContext ctx, JSONObject body) throws Exception {
    ...
    if ("POST".equals(ctx.getHttpMethod())) {
      ...
      firstname = hasFirstname
          ? StringUtils.trimToEmpty(body.optString(FIELD_FIRSTNAME, ""))
          : StringUtils.trimToEmpty(persisted[1]);
      lastname = hasLastname
          ? StringUtils.trimToEmpty(body.optString(FIELD_LASTNAME, ""))
          : StringUtils.trimToEmpty(persisted[2]);
    }
  ...
  private static String[] queryPersistedNameParts(String recordId) throws Exception {
    ...
    try (PreparedStatement ps = conn.prepareStatement(
        "SELECT name, em_etgo_firstname, em_etgo_lastname" +
        "  FROM c_bpartner WHERE c_bpartner_id = ?")) {
      ps.setString(1, recordId);
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next()) {
          return new String[]{
              StringUtils.trimToEmpty(rs.getString(1)),
