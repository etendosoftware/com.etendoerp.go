...
public class GoodsShipmentLineHandler implements NeoHandler {
  ...
  private Map<String, LineData> fetchLineData(List<String> lineIds) {
    ...
    // COALESCE: prefer order qty; fall back to invoice qty when line was imported from invoice (no order link)
    String sql =
        "SELECT il.m_inoutline_id, COALESCE(ol.qtyordered, cinvl.qtyinvoiced), p.value "
        + "FROM m_inoutline il "
